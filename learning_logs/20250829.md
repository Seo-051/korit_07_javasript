...
문제가 생김
이를 해결하기 위한 방법은? 효율적인 방법이 무엇일까

1. 01_shoppinglist.css를 만들고, 01_todolist.css의 코드를
전부 다 긁어서 복사하고, 01_shoppinglist.css에서 수정한다.
  - 혹시 수정하다가 잘못건들면 todolist 수정하기 골치 아파서

2. 01_shoppinglist.css를 만드는거 까지는 좋은데, 우리가 좀
고급 css 배우면서 했던 방식을 쓴다 -> 힌트 : 모듈화