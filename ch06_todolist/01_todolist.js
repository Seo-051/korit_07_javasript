...
function renderTodos() {
  ...
  todos.forEach((todo, index) => {
    ...
    if (todo.completed) {
      // localStorage의 'todos' key의 내부를 확인했을 때 배열로 저장되어있고, 거기 내부에 JS 객체가 있는데, 그 객체가 현재 todo라고 이름 붙여져있습니다. 그 todo들은 text라는 key, completed라는 key를 가지고 있었습니다.
...
function addTodo() {
  ...
  renderTodos(); // 추가 버튼 누르고 나서 다시 (갱신된) 리스트
  // 와야하니까 renderTodos() 함수 호출
  saveTodos(); // 그리고 localStorage에도 저장해줘야 하니까 saveTodos() 함수 호출

  // 그러면 이제 의문이 생길 수 있는게 어짜피 renderTodos()할때 saveTodos()가ㅣ 필수인것
  // 같은데 함께 묶어서 쓰면 안되나요

  // method는 하나 당 기능 하나라고 생각해야함.
  // 모듈화
}

// '추가' 버튼 클릭 이벤트
addBtn.addEventListener("click", addTodo); // button 태그에
//onclick 속성입니다. -> 두번째 argment로 addTodo 함수넣었는데
//addTodo() 가 아니라는 점 주의
